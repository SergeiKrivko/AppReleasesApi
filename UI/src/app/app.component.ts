...
export class App implements OnInit {
  ...
  private mainObservables(): Observable<undefined> {
    return merge(
      ...
      this.bundleService.loadBundlesOnApplicationChange$$,
      this.installersService.loadInstallersOnApplicationChange$,
      this.authService.isAuthorized$.pipe(
        switchMap(isAuthorized => {
          if (isAuthorized)
            return merge(
              this.applicationService.loadApplications(),
              this.installersService.loadAvailableInstallers(),
