...
) : Controller
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!await authorizationHelper.VerifyApplication(User, application))
            return Unauthorized();
        var id = await installerService.AddNewInstallerBuilderForApplicationAsync(schema.Key, applicationId,
    ...
    public async Task<ActionResult<Guid>> GetAllInstallerBuilders(Guid applicationId,
        CancellationToken cancellationToken)
    {
        var builders = await installerService.GetAllInstallerBuildersOfApplicationAsync(applicationId, cancellationToken);
        return Ok(builders);
    }
}